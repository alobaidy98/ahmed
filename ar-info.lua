   ...
   Username = '----'
    ...
    if not value then
	 if result.id == tonumber(Arian) then
	   text = text..'😍 مركزك : Executive Admin \n\n'
	  elseif is_sudo(result.id) then
	   text = text..'😍 مركزك :  المطور مالتي 😻🙊\n\n'
	  elseif is_owner(result.id, extra.chat2) then
	   text = text..'😍 مركزك :  مدير المجموعه 🌺😍\n\n'
	  elseif is_momod(result.id, extra.chat2) then
	   text = text..'😍 مركزك :  ادمن 👍🏻☺️\n\n'
      else
	   text = text..'😍 مركزك :  مجرد عضو 😒💔 \n\n'
	 end
   else
   text = text..'😍 مركزك : '..value..'\n\n'
  end
