...
local function kick_ban_res(extra, success, result)
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "دي" then
         if member_id == from_id then
            send_large_msg(receiver, "متكدر تطرد نفسك")
			return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "متكدر تطرد الادمنية والمدراء")
			return
         end
      ...
      elseif get_cmd == 'الغاء الحظر' then
        send_large_msg(receiver, 'العضو @'..member..' ['..member_id..'] ضوج يالشامت راح الحضر منه')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'العضو '..user_id..' ضوج يالشامت راح الحضر منه'
      elseif get_cmd == 'بلوك عام' then
        send_large_msg(receiver, 'العضو @'..member..' ['..member_id..'] تم نحضر عام ')
...
if matches[1]:lower() == 'دي' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "متكدر تطرد الادمنية والمدراء"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "متكدر تطرد نفسك"
		end
    local user_id = matches[2]
    local chat_id = msg.to.id
    print("sexy")
  ...
  if matches[1]:lower() == 'بلوك عام' and is_admin1(msg) then -- Global ban
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'العضو ['..user_id..' ] تم نحضر عام'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'بلوك عام',
