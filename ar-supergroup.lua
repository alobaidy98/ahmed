...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return ' تم ✔️ اقفل الروابط في المجموعه 🔐'
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return ' تم ✔️ افتح الروابط في المجموعه يمكنك الارسال الان 🔓'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return '😠 التكرار بالفعل ماقفل 🔐✔️'
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم ✔️ اقفل التكرار 🔐'
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم ✔️ اقفل اللغه العربيه 🔐'
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم ✔️ افتح اللغه العربيه 🔓'
  end
end
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم ✔️ اقفل الاضافه الجماعيه 🔐 ✋🏻'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم ✔️ افتح الاضافه الجماعيه 🔓🌹'
  end
end
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    ...
    save_data(_config.moderation.data, data)
    return 'تم ✔️ افتح جهات الاتصال 🔓⭕️'
  end
end
...
function get_message_callback(extra, success, result)
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
			   return send_large_msg("channel#id"..channel_id, "❌ لتمضرط بكيفك متكدر تطرد ادمن او مديرَ❗️")
    end
    if is_admin2(member_id) then
         return send_large_msg("channel#id"..channel_id, "❌ لتمضرط بكيفك متكدر تطرد اداري❗️")
    end
	...
	elseif get_cmd == "ارفع اداري" then
		...
		if result.from.username then
			text = "@"..result.from.username.." بَعـدَ شـتريَدْ 😍 مَنَ ربـَكَ تَمَ ارفعَكَ فَي ألآَدرهَ 💪"
		else
			text = "[ "..user_id.." ]بَعـدَ شـتريَدْ 😍 مَنَ ربـَكَ تَمَ ارفعَكَ فَي ألآَدرهَ 💪"
		end
	...
	elseif get_cmd == "نزل اداري" then
		...
		if result.from.username then
			text = "@"..result.from.username.." خطـيهَ تمَ نزله 😢 مَنْ ألآَدأرهَ لتبجي جرَأَرهَ كلـبيَ 💔"
		else
			text = "[ "..user_id.." ] خطـيهَ تمَ نزله 😢 مَنْ ألآَدأرهَ لتبجي جرَأَرهَ كلـبيَ 💔"
		end
	...
	elseif get_cmd == "ارفع المدير" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				text = "@"..result.from.username.." [ "..result.from.peer_id.." ] تم ضلعي روح صرت مدير  👍 سفنجهَ كلـبيَ 💔"
			else
				text = "[ "..result.from.peer_id.." ] تم ضلعي روح صرت مدير  👍 سفنجهَ كلـبيَ 💔"
			end
			send_large_msg(channel_id, text)
		end
	...
	elseif get_cmd == 'mute_user' then
		...
		elseif is_admin1(msg) then
			...
			send_large_msg(receiver, " ["..user_id.."] ✋🏻ارتاح هسه لصموك 🤐")
		end
	end
end
...
local function cb_user_info(extra, success, result)
	...
	if get_cmd == "نزل اداري" then
		...
		if result.username then
			text = "@"..result.username.." خطـيهَ تمَ نزله 😢 منْ ألآَدأرهَ لتبجي جرَأَرهَ كلـبيَ 💔"
			send_large_msg(receiver, text)
		else
			text = "[ "..result.peer_id.." ] خطـيهَ تمَ نزله 😢 منْ ألآَدأرهَ لتبجي جرَأَرهَ كلـبيَ 💔"
			send_large_msg(receiver, text)
		end
	elseif get_cmd == "ارفع ادمن" then
		if result.username then
...
elseif get_cmd == "ارفع اداري" then
   for k,v in pairs(result) do
    ...
    if vusername == member or vpeer_id == memberid then
      ...
      channel_set_admin(channel_id, user_id, ok_cb, false)
      if v.username then
        text = "@"..v.username.." ["..v.peer_id.."] بَعـدَ شـتريَدْ 😍 منَ ربـَكَ تَمَ ارفعَكَ فَي ألآَدرهَ 💪"
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] set admin @"..v.username.." ["..v.peer_id.."]")
      else
        text = "["..v.peer_id.."] بَعـدَ شـتريَدْ 😍 منَ ربـَكَ تَمَ ارفعَكَ فَي ألآَدرهَ 💪"
	...
	for k,v in pairs(result) do
		...
		if vusername == member or vpeer_id == memberid then
			...
			if group_owner then
				...
				if result.username then
					text = member_username.." ["..v.peer_id.."] تم ضلعي روح صرت مدير  👍 سفنجهَ كلـبيَ 💔"
				else
					text = "["..v.peer_id.."] تم ضلعي روح صرت مدير  👍 سفنجهَ كلـبيَ 💔"
				end
			end
		elseif memberid and vusername ~= member and vpeer_id ~= memberid then
			...
			if group_owner then
				...
				text = "["..memberid.."] تم ضلعي روح صرت مدير  👍 سفنجهَ كلـبيَ 💔"
			end
		end
	end
 end
...
local function run(msg, matches)
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
		if matches[1] == 'فعل مجموعه' and not matches[2] then
		...
		if matches[1] == 'عطل مجموعه' and is_admin1(msg) and not matches[2] then
